...
exports.addSP = async (req, res) => {
    try {
      const { name,vehicleNumber,brand,model,year,color,contactNumber,description} =
        req.body;
      ...
      if (typeof Year !== "number" || isNaN(Year) || Year <= 1600) {
        return res
          .status(400)
          .json({ error: "year must be a positive year" });
      }
  ...
  exports.SPpendingorders = async (req, res) => {
    try {
      const Sp = await spSchema.find().sort({ createdAt: -1 });
      res.json(Sp);
    } catch (err) {
      console.log(err);
    }
  ...
  exports.deletependingsp = async (req, res) => {
    ...
    try {
      ...
      const imagePath = pendingsp.image;
